...
class VoiceCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot, audio_manager: AudioManager, bot_state_manager: BotState,
        ...
        self.voice_client: Optional[voice_recv.VoiceRecvClient] = None  # Current voice connection
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, user: discord.User) -> None:
        ...
        if self.bot_state_manager.standby_message and reaction.message.id == self.bot_state_manager.standby_message.id:
            ...
            # Handle ❌ reaction to cancel recording
                    ...
                    reaction.emoji == "❌" and 
                    self.bot_state_manager.current_state == BotStateEnum.RECORDING and 
                    self.bot_state_manager.is_authorized(user)
            ):
    ...
    async def on_reaction_remove(self, reaction: discord.Reaction, user: discord.User) -> None:
        ...
        if (
                self.bot_state_manager.standby_message and 
                reaction.message.id == self.bot_state_manager.standby_message.id and 
                reaction.emoji == "🎙" and 
                self.bot_state_manager.current_state == BotStateEnum.RECORDING and 
                self.bot_state_manager.is_authorized(user)
        ):
