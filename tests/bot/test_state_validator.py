...
class TestStateTransitionValidator:
    ...
    def test_valid_transitions_from_standby(self):
        ...
        StateTransitionValidator.validate(BotStateEnum.STANDBY, BotStateEnum.CONNECTION_ERROR)
    ...
    def test_valid_transitions_from_recording(self):
        ...
        StateTransitionValidator.validate(BotStateEnum.RECORDING, BotStateEnum.CONNECTION_ERROR)
    ...
    def test_valid_transitions_from_listening(self):
        ...
        StateTransitionValidator.validate(BotStateEnum.LISTENING, BotStateEnum.CONNECTION_ERROR)
    ...
    def test_valid_transitions_from_speaking(self):
        ...
        StateTransitionValidator.validate(BotStateEnum.SPEAKING, BotStateEnum.CONNECTION_ERROR)
    ...
    def test_valid_transitions_from_connection_error(self):
        ...
        StateTransitionValidator.validate(BotStateEnum.CONNECTION_ERROR, BotStateEnum.STANDBY)
        
        # CONNECTION_ERROR -> LISTENING (recovery to realtime mode)
        StateTransitionValidator.validate(BotStateEnum.CONNECTION_ERROR, BotStateEnum.LISTENING)
        
        # CONNECTION_ERROR -> IDLE (disconnect/give up)
        StateTransitionValidator.validate(BotStateEnum.CONNECTION_ERROR, BotStateEnum.IDLE)
    ...
    def test_invalid_transitions_from_idle(self):
        ...
        with pytest.raises(StateTransitionError):
            StateTransitionValidator.validate(BotStateEnum.IDLE, BotStateEnum.CONNECTION_ERROR)

    def test_invalid_transitions_from_standby(self):
        """Test invalid transitions from STANDBY state."""
        with pytest.raises(StateTransitionError) as exc_info:
            StateTransitionValidator.validate(BotStateEnum.STANDBY, BotStateEnum.LISTENING)
        assert "Invalid state transition from standby to listening" in str(exc_info.value)
        
        with pytest.raises(StateTransitionError):
            StateTransitionValidator.validate(BotStateEnum.STANDBY, BotStateEnum.SPEAKING)
    ...
    def test_invalid_transitions_from_recording(self):
        ...
        with pytest.raises(StateTransitionError):
            StateTransitionValidator.validate(BotStateEnum.RECORDING, BotStateEnum.LISTENING)
        
        with pytest.raises(StateTransitionError):
            StateTransitionValidator.validate(BotStateEnum.RECORDING, BotStateEnum.SPEAKING)

    def test_invalid_transitions_from_listening(self):
        """Test invalid transitions from LISTENING state."""
        with pytest.raises(StateTransitionError) as exc_info:
            StateTransitionValidator.validate(BotStateEnum.LISTENING, BotStateEnum.STANDBY)
        assert "Invalid state transition from listening to standby" in str(exc_info.value)
        
        with pytest.raises(StateTransitionError):
            StateTransitionValidator.validate(BotStateEnum.LISTENING, BotStateEnum.RECORDING)
    ...
    def test_invalid_transitions_from_speaking(self):
        ...
        with pytest.raises(StateTransitionError):
            StateTransitionValidator.validate(BotStateEnum.SPEAKING, BotStateEnum.STANDBY)
        
        with pytest.raises(StateTransitionError):
            StateTransitionValidator.validate(BotStateEnum.SPEAKING, BotStateEnum.RECORDING)

    def test_invalid_transitions_from_connection_error(self):
        """Test invalid transitions from CONNECTION_ERROR state."""
        with pytest.raises(StateTransitionError) as exc_info:
            StateTransitionValidator.validate(BotStateEnum.CONNECTION_ERROR, BotStateEnum.RECORDING)
        assert "Invalid state transition from connection_error to recording" in str(exc_info.value)
        
        with pytest.raises(StateTransitionError):
            StateTransitionValidator.validate(BotStateEnum.CONNECTION_ERROR, BotStateEnum.SPEAKING)
    ...
    def test_get_valid_transitions(self):
        ...
        # Test IDLE state
        valid_from_idle = StateTransitionValidator.get_valid_transitions(BotStateEnum.IDLE)
        expected_from_idle = {BotStateEnum.STANDBY, BotStateEnum.LISTENING}
        assert valid_from_idle == expected_from_idle
        
        # Test STANDBY state
        valid_from_standby = StateTransitionValidator.get_valid_transitions(BotStateEnum.STANDBY)
        expected_from_standby = {BotStateEnum.RECORDING, BotStateEnum.IDLE, BotStateEnum.CONNECTION_ERROR}
        assert valid_from_standby == expected_from_standby
        
        # Test RECORDING state
        valid_from_recording = StateTransitionValidator.get_valid_transitions(BotStateEnum.RECORDING)
        ...
        valid_from_listening = StateTransitionValidator.get_valid_transitions(BotStateEnum.LISTENING)
        expected_from_listening = {BotStateEnum.SPEAKING, BotStateEnum.IDLE, BotStateEnum.CONNECTION_ERROR}
        assert valid_from_listening == expected_from_listening
        
        # Test SPEAKING state
        valid_from_speaking = StateTransitionValidator.get_valid_transitions(BotStateEnum.SPEAKING)
        ...
        valid_from_error = StateTransitionValidator.get_valid_transitions(BotStateEnum.CONNECTION_ERROR)
        expected_from_error = {BotStateEnum.STANDBY, BotStateEnum.LISTENING, BotStateEnum.IDLE}
    ...
    def test_get_valid_transitions_returns_copy(self):
        ...
        valid_transitions = StateTransitionValidator.get_valid_transitions(BotStateEnum.IDLE)
        ...
        fresh_transitions = StateTransitionValidator.get_valid_transitions(BotStateEnum.IDLE)
